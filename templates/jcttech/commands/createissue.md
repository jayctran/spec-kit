...
This command creates GitHub issues using organization-defined templates from `.specify/org-templates/`. It ensures issues follow the correct structure and include appropriate labels.

## Outline

1. Run `{SCRIPT}` from repo root and parse REPO_ROOT path. All paths must be absolute.

2. Load organization templates from `$REPO_ROOT/.specify/org-templates/`:
